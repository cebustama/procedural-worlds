...
namespace ProceduralWorlds.Meshes.Generators
{
    public struct PointyHexagonGrid : IMeshGenerator
    {
        ...
        public Bounds Bounds => new Bounds(Vector3.zero, new Vector3(
            (Resolution > 1 ? 0.5f + 0.25f / Resolution : 0.5f) * sqrt(3), 
            0f,
        ...
        public void Execute<S>(int z, S streams) where S : struct, IMeshStreams
        {
            ...
            // Halve triangle size, offset by height of equilateral triangle
            float h = sqrt(3f) / 4f;
            ...
            for (int x = 0; x < Resolution; x++, vi += 7, ti += 6)
            {
                ...
                // V coordinates cover the entire 0–1 range while the U coordinates go from 
                // 0.5 - h to 0.5 + h, which is the width of the hexagon
